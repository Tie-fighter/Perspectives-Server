...
# Looks in the cache for a record and returns it if ok
# Returns False if the record is not up-to-date or missing
def	query_cache(service_id, end):
	# Check if entry exists
	#print 'Querying cache...'
	if (service_id in cache):
		#print 'Cache hit!'
		# Check if saved end is older than current end
		if (int(cache[service_id][0]) < int(end)):
			#print 'Cache not up-to-date'
			return False
		else:
			return cache[service_id][1]
	else:
		#print 'Cache miss!'
		return False

# Inserts/Updates a record in the cache
# Returns nothing
def update_cache(service_id, end, xml):
	cache[service_id] = [int(end), xml]
	#print 'Cache updated!'
	return
...
class NotaryHTTPServer:
	...
	def get_xml(self, service_id): 
		...
		# get youngest timerecord
		cur.execute("SELECT end FROM observations WHERE service_id = ? AND key not NULL ORDER BY end DESC LIMIT 1", (service_id,))
		end = cur.fetchone()[0]

		# query the cache
		xml = query_cache(service_id, end)
		if (xml == False):
			...
			# update the cache
			update_cache(service_id, end, top_element.toprettyxml())
